...
/**Because the 'members' schema on our database contains the userId but not the userName of every member, if we want
 * to display the list of all members in a way the user can understand, we need a helper method to go into the db and
 * connect the members table's userId to the users table's userId field, thus making the search work properly.
 */
...
export const remove = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const [messages, reactions, conversations] = await Promise.all([
            ...
            .filter((q) => q.or(
                q.eq(
                    q.field('memberOneId'), member._id
                ),
                q.eq(
                    q.field('memberTwoId'), member._id
                )
            ))
            .collect(),
        ])
